...
if __name__ == "__main__":
    ...
    try:
        ...
        op = SystemOperation(
            SystemOperation.OpCode.CreateTask, ("./examples/resnet50.py", [1])
        )
        logger.info(f"Requesting: {op}")
        zmq_client.send(op)
        response = zmq_client.recv()
        ...
        input("Press any key to continue test if workers continued successfully...\n")
        ...
        logger.info(f"Requesting: {op}")
        zmq_client.send(op)
        response = zmq_client.recv()  # todo: 支持返回数据
        logger.info(f"Response from hub: {response}")
