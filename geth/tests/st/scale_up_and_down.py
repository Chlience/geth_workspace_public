...
if __name__ == "__main__":
    ...
    try:
        ...
        op = SystemOperation(
            SystemOperation.OpCode.CreateTask, ("./examples/resnet50.py", [0])
        )
        ...
        op = SystemOperation(SystemOperation.OpCode.UpdateTask, (0, [1]))
        logger.info(f"Requesting: {op}")
        zmq_client.send(op)
        response = zmq_client.recv()
        logger.info(f"Response from hub: {response}")
        ...
        input("Press any key to test system still functioning...\n")
        op = SystemOperation(
            SystemOperation.OpCode.CreateTask, ("./examples/resnet50.py", [0, 1])
        )
        logger.info(f"Requesting: {op}")
        zmq_client.send(op)
        response = zmq_client.recv()
        ...
        op = SystemOperation(SystemOperation.OpCode.UpdateTask, (1, [1]))
        logger.info(f"Requesting: {op}")
        zmq_client.send(op)
        response = zmq_client.recv()
        logger.info(f"Response from hub: {response}")
        ...
        op = SystemOperation(SystemOperation.OpCode.UpdateTask, (1, [0, 1]))
        logger.info(f"Requesting: {op}")
        zmq_client.send(op)
        response = zmq_client.recv()
        logger.info(f"Response from hub: {response}")
