...
from .elastic_with_prestart import BEFORE_SCALE_LIMIT

STOP_SCALE_LIMIT = 10
PRE_START_LIMIT = STOP_SCALE_LIMIT + 5

class ElasticPrestartSjfScheduler(Scheduler):
    ...
    def schedule(self) -> None:
        ...
        for machine_id, machine in self.cluster.machines.items():
            ...
            # Re-sort queue by remaining time after removing scheduled jobs
            self.queue.sort(key=lambda j: j.get_remaining_time(j.requirements.min_gpus))
            scale_in_flag = False
            ...
            if not new_job_flag and len(available_gpus) > 0 and self.queue:
                job_disadvantages = []
                # wsq select the job with shortest remaining time (SJF)
                pending_job = min(self.queue, key=lambda j: j.get_remaining_time(j.requirements.min_gpus))
